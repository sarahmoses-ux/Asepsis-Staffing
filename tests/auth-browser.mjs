...
const server = spawn(process.execPath,['server/index.js'],{env:{...process.env,PORT:'5192',DATA_DIR:`.artifacts/browser-db-${Date.now()}`},windowsHide:true,stdio:'pipe'});
let browser;
try {
  ...
  await page.goto(origin+'/#account'); await expect(page.getByText('Reference #1',{exact:false})).toBeVisible();
