...
const directory = `.artifacts/auth-${Date.now()}`;
mkdirSync(directory, { recursive: true });
let server;
async function start() {
  server = spawn(process.execPath, ['server/index.js'], { env: { ...process.env, PORT: '5191', DATA_DIR: directory }, windowsHide: true, stdio: ['ignore','pipe','pipe'] });
