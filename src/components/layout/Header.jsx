...
export default function Header({ screen, goHome, goJobs, goRequest, goApply, goWorker, goEmployer, goSection, savedCount, user, goLogin, goSignup, goAccount, logout }) {
    ...
    <header className="site-header">
      <div className="container header-inner">
        ...
        <div className="header-actions">
          {user ? <div className="account" ref={accountRef}>
            ...
            {accountOpen && (
              <div className="account-panel" id="account-links">
                <button onClick={navigate(goAccount)}>My dashboard</button>
                {user.role === 'worker' && <button onClick={navigate(goAccount)}>Saved jobs <span>{savedCount}</span></button>}
                <button onClick={navigate(logout)}>Log out</button>
              </div>
            )}
          </div> : <button className="account-toggle" onClick={navigate(goLogin)}>Log in</button>}
          <button className="button button--primary" onClick={navigate(user ? goAccount : goSignup)}>{user ? 'My dashboard' : 'Sign up'}</button>
