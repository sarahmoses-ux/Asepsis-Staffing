...
export default function AccountScreen({ user, records, savedJobs, goJobs, goApply, goRequest, accountLoading, reloadAccount }) {
  ...
  return <section className="account-dashboard container"><span className="account-eyebrow">{employer ? 'EMPLOYER ACCOUNT' : 'WORKER ACCOUNT'}</span><h1>Hello, {user.name.split(' ')[0]}.</h1><p>Your next steps, all in one place.</p>
    <div className="dashboard-actions"><button className="button button--primary" onClick={employer ? goRequest : goJobs}>{employer ? 'Request staff' : 'Find work'}</button>{!employer && <button className="button" onClick={goApply}>Start an application</button>}<button className="button" onClick={reloadAccount} disabled={accountLoading}>{accountLoading ? 'Refreshing…' : 'Refresh'}</button></div>
